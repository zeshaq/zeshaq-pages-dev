...
| **P4: Redis client + Trivy + CI/CD glue** | Day 10-12 | OCP-side Redis mock client connecting to the existing VM Sentinel pool; SBOM pipeline extension; BRAC monorepo + conditional Jenkinsfile; hybrid CD to OCP + one VM target | Hard-kill failover demo (VM-side, with OCP client reconnect); DefectDojo SBOM view; monorepo-aware build triggers only the affected app |
...
6. **Redis** (existing VM Sentinel + OCP-side mock client): hard-kill the primary on the VM side; tail the mock client's logs (running on OCP) showing it reconnects through Sentinel to the newly-promoted primary.
