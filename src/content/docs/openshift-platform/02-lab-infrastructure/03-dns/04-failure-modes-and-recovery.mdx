...
3. **Replay any record changes** made since the backup. The reference is `opp-full-plat/plans/disconnected-rebuild/environments/dc-lab/dns-records.md` — every record addition is supposed to be journaled there with a serial number, so you can replay from the diff between the backup serial and `HEAD`.
