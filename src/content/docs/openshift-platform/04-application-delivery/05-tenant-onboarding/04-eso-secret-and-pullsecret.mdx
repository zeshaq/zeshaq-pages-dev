...
The `ClusterExternalSecret` for the cluster-wide pull Secret has no role here — runtime pull and CI push are different concerns with different credentials. See [09 — Per-tenant Quay robot token](./09-quay-robot-token-per-tenant) for the full pattern.
