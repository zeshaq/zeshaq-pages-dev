...
description: "The REP-* tracker — promoting codex-opp-agent + dc-lab into a replicable site-builder framework: profile schema, GitLab bootstrap, scripts refactor, template generators, and the dry-run gate."
...
Site Replication Readiness (milestone [#31](https://github.com/zeshaq/codex-opp-agent/milestone/31)) is the long-running effort to promote the `codex-opp-agent` workspace + the dc-lab instance into a **replicable site-builder framework**: another team takes the package, runs it against their site profile, ends up with the same platform shape. This page documents what the framework is, where it stands today, the REP-* tracker, and the gap between "framework exists" and "framework proven."

The PCI-DSS baseline that closed on 2026-05-11 is the *first* repeatable end-to-end exercise on the v6 fleet. REP-* is what makes the *next* site repeat it without reaching for chat history or undocumented memory.
...
| --- | --- | --- |
| [REP-0 #144](https://github.com/zeshaq/codex-opp-agent/issues/144) | OPEN (long-running parent) | n/a |
| [REP-1 #145](https://github.com/zeshaq/codex-opp-agent/issues/145) — profile schema + dc-lab extraction | **CLOSED** | `plans/disconnected-rebuild/site-profile-schema.md` + `plans/disconnected-rebuild/environments/dc-lab/profile.yaml` + `scripts/validate-profile.py` (295/295 PASS) |
| [REP-2 #146](https://github.com/zeshaq/codex-opp-agent/issues/146) — parameterize handoffs | **CLOSED (Phase 1)** | `scripts/render-handoffs.py` + 3 handoffs as `.j2` templates; dc-lab byte-clean render |
| [REP-2.1 #241](https://github.com/zeshaq/codex-opp-agent/issues/241) — parameterize remaining docs | OPEN | RUNBOOK.md + remaining `connection-details/` |
| [REP-2.2 #243](https://github.com/zeshaq/codex-opp-agent/issues/243) — unify the two profile.yaml files | **CLOSED** | Single canonical profile at `environments/<site>/profile.yaml`; schema doc updated |
| [REP-3 #147](https://github.com/zeshaq/codex-opp-agent/issues/147) — GitLab content bootstrap | OPEN (Phase 1) | `plans/disconnected-rebuild/gitlab-bootstrap-design.md` + `terraform/gitlab-bootstrap/` + `scripts/gitlab/render-codeowners.py` |
| [REP-3.2/3.3/3.4 #235/#236/#237](https://github.com/zeshaq/codex-opp-agent/issues/235) | OPEN | Role groups + shares + branch protection / runner registration / full repo seeding |
| [REP-4 #148](https://github.com/zeshaq/codex-opp-agent/issues/148) — `scripts/rebuild/` profile-aware refactor | OPEN (Phase 1) | `scripts/rebuild/REFACTOR-PATTERN.md` + `lib/profile.sh` + 3 reference refactors; 11/11 smoke PASS |
| [REP-4.2 #244](https://github.com/zeshaq/codex-opp-agent/issues/244) — refactor remaining ~22 scripts | OPEN | n/a (large mechanical pass) |
| [REP-5 #149](https://github.com/zeshaq/codex-opp-agent/issues/149) — `make new-site` generator | OPEN (Phase 1) | `Makefile` + `scripts/new-site.py` + `templates/issues/r-chain/` R0-R6 |
| [REP-5.2/3/4/5 #238/#239/#240/#242](https://github.com/zeshaq/codex-opp-agent/issues/238) | OPEN | L0-L7 / PCI / FG+DH chains + GitHub Projects v2 GraphQL |
| [REP-6 #150](https://github.com/zeshaq/codex-opp-agent/issues/150) — memory → runbooks | **CLOSED** | 5 memory entries migrated to `runbooks/*` + `connection-details/signoz.md` |
| [REP-7 #151](https://github.com/zeshaq/codex-opp-agent/issues/151) — dry-run rebuild | OPEN (BLOCKED — proof gate) | Comment listing preconditions |
...
Phase-1 acceptance: render the templates against `dc-lab/profile.yaml` and diff against today's checked-in handoff docs. The diff must be empty (or only cosmetic whitespace). This was achieved. Remaining work in REP-2.1: `RUNBOOK.md` + the per-service connection-details docs (`nexus.md`, `jenkins.md`, `minio.md`, `vault-app-secrets.md`, etc.) still need conversion.
...
Two YAMLs for the same site = drift trap. REP-2.2 unified them at `environments/<site>/profile.yaml` as the canonical path, extending the REP-1 schema to absorb REP-2's extra sections. The old path was retired with a deprecation note.

## REP-3 — GitLab content bootstrap (open, Phase 1)
...
- [REP-3.2 #235](https://github.com/zeshaq/codex-opp-agent/issues/235) — role groups + project shares + branch protection via Terraform.
- [REP-3.3 #236](https://github.com/zeshaq/codex-opp-agent/issues/236) — runner registration token generation + Vault storage.
- [REP-3.4 #237](https://github.com/zeshaq/codex-opp-agent/issues/237) — full repo seeding (infra-ops, platform-services, tenant-registry, divisions).
...
Smoke test `scripts/rebuild/lib/test-profile-lib.sh` passes 11/11. The remaining ~22 scripts are tracked under [REP-4.2 #244](https://github.com/zeshaq/codex-opp-agent/issues/244) as a mechanical pass.
...
- [REP-5.2 #238](https://github.com/zeshaq/codex-opp-agent/issues/238) — L0-L7 chain templates.
- [REP-5.3 #239](https://github.com/zeshaq/codex-opp-agent/issues/239) — PCI-0..PCI-5 chain templates.
- [REP-5.4 #240](https://github.com/zeshaq/codex-opp-agent/issues/240) — FG-0..FG-6 + DH chain templates.
- [REP-5.5 #242](https://github.com/zeshaq/codex-opp-agent/issues/242) — GitHub Projects v2 GraphQL integration.

## REP-6 — memory → runbooks (closed)

Claude's per-session memory under `claude-agent/memory/project_*` is invisible to another operator. For replicability, the lessons had to migrate into tracked codex-opp-agent docs.

Closed at commit `c27dd5f`. Migrated artifacts:

| Memory entry | Migrated to |
| --- | --- |
...
- **Zero "ask Claude" moments** during execution; every step traceable to a checked-in artifact.

REP-7 is **blocked on:** REP-2.1 (remaining handoffs), REP-3.2/3.3/3.4 (full GitLab bootstrap), REP-4.2 (remaining scripts), REP-5.2/3/4/5 (remaining chain templates), AND test infrastructure availability.
...
| Handoff rendering (REP-2 Phase 1) | Yes for 3 handoffs | REP-2.1: RUNBOOK.md + per-service connection-details |
| GitLab bootstrap automation | Design + module skeleton | Full TF modules (REP-3.2/3.3/3.4) |
| `scripts/rebuild/` profile-aware | Pattern + 3 refactors | Remaining ~22 scripts (REP-4.2) |
| `make new-site` | Skeleton + R-chain templates | L/PCI/FG+DH chains; GitHub Projects v2 (REP-5.2-5.5) |
...
Next visible chunks (per REP-0 status, post-Wave 3):

1. **REP-2.1** — close out remaining handoff/connection-details parameterization. Mostly mechanical.
2. **REP-3.2 → 3.3 → 3.4** — complete the GitLab bootstrap Terraform; this is the second-largest remaining body of work.
3. **REP-4.2** — mechanical refactor of the remaining `scripts/rebuild/*` to consume the profile. The pattern + helpers exist; this is "apply the pattern to each script."
4. **REP-5.2 / 5.3 / 5.4 / 5.5** — fill out the chain templates and wire the Projects v2 GraphQL integration so `make new-site` produces a fully-populated project board.
5. **Test infra** — REP-7 needs hardware. Could be budget-friendly Proxmox or cloud VMs.

REP-0 itself remains open as the long-running parent issue.

## References

- REP-0 [#144](https://github.com/zeshaq/codex-opp-agent/issues/144) — meta + strategy
- REP-1 [#145](https://github.com/zeshaq/codex-opp-agent/issues/145) — profile schema (CLOSED)
- REP-2 [#146](https://github.com/zeshaq/codex-opp-agent/issues/146) — handoff templates (CLOSED Phase 1)
- REP-2.1 [#241](https://github.com/zeshaq/codex-opp-agent/issues/241) — remaining handoff parameterization
- REP-2.2 [#243](https://github.com/zeshaq/codex-opp-agent/issues/243) — profile unification (CLOSED)
- REP-3 [#147](https://github.com/zeshaq/codex-opp-agent/issues/147) — GitLab bootstrap (OPEN Phase 1)
- REP-3.2/3.3/3.4 [#235/#236/#237](https://github.com/zeshaq/codex-opp-agent/issues/235)
- REP-4 [#148](https://github.com/zeshaq/codex-opp-agent/issues/148) — scripts refactor (OPEN Phase 1)
- REP-4.2 [#244](https://github.com/zeshaq/codex-opp-agent/issues/244) — remaining scripts
- REP-5 [#149](https://github.com/zeshaq/codex-opp-agent/issues/149) — `make new-site` (OPEN Phase 1)
- REP-5.2/3/4/5 [#238/#239/#240/#242](https://github.com/zeshaq/codex-opp-agent/issues/238)
- REP-6 [#150](https://github.com/zeshaq/codex-opp-agent/issues/150) — memory → runbooks (CLOSED)
- REP-7 [#151](https://github.com/zeshaq/codex-opp-agent/issues/151) — dry-run rebuild (BLOCKED)
- Milestone [Site Replication Readiness #31](https://github.com/zeshaq/codex-opp-agent/milestone/31)
- ADR 0001, 0015, 0018, 0019, 0022, 0023, 0024 — the foundational decisions the framework relies on
- Memory `project_obc_to_operand_secret_bridge.md` — OBC → operand storage-Secret bridge
- Memory `reference_rhacs_init_bundle_via_api.md` — RHACS init-bundle pattern
