...
ADR 0001 was accepted **2026-05-04**. It defined the operator workspace at `/home/ze/codex-opp-agent` and named the fleet:
...
4. **Pre-v6 purge (2026-05-10).** Issue [#156](https://github.com/zeshaq/codex-opp-agent/issues/156) OPS-V6-FLEET-1 + ADR 0022 formalized that pre-v6 names are decommissioned. `CLUSTERS.md` updated, plans updated, memory updated.
...
- `codex-opp-agent` workspace structure — every file under `adr/`, `plans/`, `scripts/rebuild/`, `connection-details/`, `runbooks/`, `reports/sessions/` was kept and is still authoritative.
...
Role-groups (`ct-*` naming) and CODEOWNERS govern who can merge what. The full pattern is documented in `gitlab-operator-guide.md` and tracked under [#83](https://github.com/zeshaq/codex-opp-agent/issues/83) "federated GitOps architecture readiness."
...
- `codex-opp-agent/CLUSTERS.md` — `hub-dr` and `spoke-dr` sections removed; Active Scope updated to name `hub-dc-v6` / `spoke-dc-v6`. ✓
- `codex-opp-agent/plans/federated-gitops-readiness-gates.md` — references to "future `hub-dr`" / "future `spoke-dr`" updated to "future `hub-dr-v6`" / "future `spoke-dr-v6`". ✓
- ADR Index issue [#131](https://github.com/zeshaq/codex-opp-agent/issues/131) — ADR 0022 added; note on ADR 0001 that the cluster list is superseded by 0022. ✓
- Workspace memory at `/home/ze/.claude/projects/-home-ze-claude-agent/memory/` updated to v6-only scope with explicit decommissioned-DR guard (`project_workspace_scope.md`). ✓
- ADR 0001 itself **not** modified — historical record preserved. ✓

The day-wrap issue [#159](https://github.com/zeshaq/codex-opp-agent/issues/159) (2026-05-10) records both ADRs accepted that day: 0020 (PCI-DSS baseline) and 0022 (v6 fleet membership). Counter: 16 MRs merged to `platform-gitops` `main`, 2 MRs merged to `codex-opp-agent` `main`, 15 issues closed.
...
- Issue [#156](https://github.com/zeshaq/codex-opp-agent/issues/156) OPS-V6-FLEET-1 — the purge work
- Issue [#131](https://github.com/zeshaq/codex-opp-agent/issues/131) — ADR Index (carries the supersession note on 0001)
- Issue [#159](https://github.com/zeshaq/codex-opp-agent/issues/159) — 2026-05-10 day wrap (records both ADRs accepted that day)
...
- Memory `project_workspace_scope.md` — v6-only scope; pre-v6 DR decommissioned
- `codex-opp-agent/CLUSTERS.md` — current fleet inventory
