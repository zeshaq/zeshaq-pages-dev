...
The helper script `claude-agent/scripts/vault-eso-wire.sh` is idempotent and creates both the auth mount and the role/policy on each cluster's onboarding.
