...
Per [issue #172 (DEV-OCP-0.2)](https://github.com/zeshaq/codex-opp-agent/issues/172).
...
The `ClusterExternalSecret` for the cluster-wide pull Secret has no role here — runtime pull and CI push are different concerns with different credentials. See [memory: per-tenant Quay robot token convention](https://github.com/zeshaq/codex-opp-agent/blob/main/connection-details/vault-app-secrets.md) for the full pattern.
...
- `codex-opp-agent/connection-details/app-registry-pullsecret.md` (issue #172, DEV-OCP-0.2) — full design.
- `codex-opp-agent/connection-details/vault-app-secrets.md` (issue #174, DEV-OCP-0.4) — per-tenant SecretStore.
