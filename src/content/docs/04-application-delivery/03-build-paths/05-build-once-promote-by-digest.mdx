...
That is the entire diff. The script (`claude-agent/scripts/update-overlay-digest.sh <team> <app> <env> <digest>`) implements:
...
```bash
claude-agent/scripts/promote.sh <team> <app> <from-env> <to-env>
