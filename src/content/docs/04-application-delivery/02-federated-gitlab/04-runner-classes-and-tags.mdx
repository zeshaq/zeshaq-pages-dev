...
This is the bootstrap runner and explicitly **not** the future trusted platform runner. The runner side of the access model is enforced by convention only until the five-class set is created. Tracked under the `GitLab Runner Operating Model` milestone in the codex-opp-agent repo.
...
- in local custody under `/home/ze/codex-opp-agent/secrets/jenkins/git-notifycommit-<project>-token.json` (Git-ignored)
...
- **Local-only secret custody** during lab bootstrap. The standard path is `/home/ze/codex-opp-agent/secrets/`, Git-ignored, mode-restricted.
...
|---|---|---|
| Bootstrap GitLab admin PAT | `/home/ze/codex-opp-agent/secrets/gitlab-md-pat` | Local-only, Git-ignored; used by operator scripts. |
| Operator GitLab PAT | `/home/ze/codex-opp-agent/secrets/gitlab-zahid-pat` | Local-only, Git-ignored; used for platform-gitops MR creation. |
| Jenkins admin credential | `/home/ze/codex-opp-agent/secrets/jenkins/admin.env` | Local-only, Git-ignored. |
