...
- in local custody under `codex-opp-agent/secrets/` as a per-project token file (local-only, Git-ignored)
...
- **Local-only secret custody** during lab bootstrap. The standard path is `codex-opp-agent/secrets/` (local-only, contents not enumerated here), Git-ignored, mode-restricted.
...
| Jenkins admin credential | `codex-opp-agent/secrets/jenkins/admin.env` (local-only) | Local-only, Git-ignored. |
