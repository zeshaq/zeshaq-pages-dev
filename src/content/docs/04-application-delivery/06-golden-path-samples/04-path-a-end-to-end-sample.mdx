...
<Mermaid client:load chart={`sequenceDiagram
    ...
    participant Agt as jenkins-agent-0
    participant Trv as Trivy server
    participant Nx as Nexus app-registry
    participant Mn as MinIO
    participant Mr as overlay MR
    participant Argo as Hub Argo
    participant Sp as Spoke Argo
    participant K as Kubelet
    Dev->>GL: git push main (t=0s)
    GL->>Jen: notifyCommit webhook (t+5s)
    Jen->>Agt: schedule build on developer-build agent
    Agt->>Nx: podman pull base (cached: ~5s)
    Agt->>Agt: podman build (t+30s..t+90s)
    Agt->>Trv: trivy image --severity CRITICAL (t+90s..t+120s)
    Agt->>Nx: podman push (t+120s..t+150s)
    Agt->>Mn: mc cp build.log + sbom + scan (t+150s..t+160s)
    Agt->>Mr: gitlab API: open MR with overlay patch (t+165s)
    Mr->>Mr: CODEOWNERS auto-merge / reviewer merges (t+200s)
    Mr->>Argo: AppSet refresh detects new main (t+200s..t+380s)
    Argo->>Sp: ManifestWork (t+380s..t+400s)
    Sp->>K: apply Deployment with new digest
    K->>K: pull image + roll out (t+400s..t+430s)
    Dev->>Dev: curl /health (t+430s, 200 OK)
`} />
