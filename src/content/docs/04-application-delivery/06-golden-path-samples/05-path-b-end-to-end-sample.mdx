...
Per [issue #203 (DEV-OCP-5.4)](https://github.com/zeshaq/codex-opp-agent/issues/203) and [issue #205 (DEV-OCP-5.6)](https://github.com/zeshaq/codex-opp-agent/issues/205), combined into one sample.
...
<Mermaid client:load chart={`sequenceDiagram
    ...
    participant Bld as buildah Task
    participant Trv as trivy-scan Task
    participant Qy as Quay
    ...
    GL->>EL: webhook (t+2s)
    EL->>PR: create PipelineRun (t+3s)
    PR->>PR: git-clone (t+3s..t+10s)
    PR->>Bld: buildah build (t+10s..t+70s)
    PR->>Trv: trivy image --severity CRITICAL (t+70s..t+100s)
    PR->>Qy: push image (t+100s..t+115s)
    PR->>Mn: evidence upload (t+115s..t+120s)
    PR->>Mr: open MR via GitLab API (t+120s..t+125s)
    Mr->>Mr: auto-merge (t+150s)
    Mr->>Argo: refresh detects new main (t+150s..t+330s)
    Argo->>K: apply Deployment
    K->>Qy: pull image via quay-pull Secret
    K->>K: roll out (t+330s..t+360s)
    Dev->>Dev: curl /health (t+360s, 200 OK)
`} />
...
- `codex-opp-agent/connection-details/build-path-matrix.md` — Path A vs Path B decision.
- `codex-opp-agent/connection-details/ci-evidence-schema.md` — cross-path evidence parity.
- Memory `reference_quay_robot_token_convention.md` — per-tenant Quay robot token pattern.
