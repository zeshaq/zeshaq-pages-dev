...
The HAProxy edge VM is the single TLS terminator for the lab's *platform* VM fleet — Jenkins, SigNoz, Trivy, DefectDojo, MinIO console, Nexus (all three Docker endpoints), GitLab, WSO2, monitoring-0/Grafana, and the rest. It is intentionally **not** the edge for OpenShift cluster traffic; that goes through the in-cluster OpenShift router (per ADR 0005 and the `feedback_haproxy_scope.md` memory). This page covers what HAProxy does at the architectural level; subsequent pages drill into frontends, the SNI passthrough pattern, backend conventions, and the certificate model.
...
- `codex-opp-agent/connection-details/platform-admin-handoff.md` (HAProxy backend identities, public hostnames)
