...
<Mermaid client:load chart={`flowchart LR
  gitlab["GitLab repo\n(developer push)"] -->|webhook| jenkins["Jenkins job"]
  jenkins -->|pull base| dgr["docker-group.apps.\nsub.comptech-lab.com"]
  jenkins -->|build| local["Local image\non build agent"]
  local -->|trivy --server| trivy["Trivy server VM"]
  trivy -->|HIGH or CRITICAL gate| gate{{"Scan passes?"}}
  gate -->|yes| push["podman push to\napp-registry.apps.*"]
  gate -->|no| fail["fail build,\nreport to MinIO"]
  push -->|evidence + report| minio["MinIO\ndeveloper-ci-evidence"]
`} />
