...
| --- | --- | --- |
| **Operator PAT** | API access for the bootstrap operator; create groups/projects/shares; scoped fine-grained per ADR direction | `secrets/gitlab-md-pat` (Git-ignored) |
...
- The PAT lives only in `secrets/gitlab-md-pat`. The directory is Git-ignored and mode-restricted (`0700`).
...
**Fix.** Revoke the PAT immediately in GitLab UI. Create a new PAT. Update `secrets/gitlab-md-pat`. Audit any tools that may have cached the old value.
