...
2. **MAC follows IP.** Each platform VM gets a MAC of the form `52:54:00:XX:XX:<HH>` where `<HH>` is the last octet of the platform-`/24` IP. The byte is written in the same base on both sides — easy to grep, easy to verify. The exact middle bytes are part of the internal lab convention recorded in `codex-opp-agent/connection-details/`.
...
OpenShift master VMs follow the same convention with a different middle segment per cluster — exact MAC patterns are in `codex-opp-agent/connection-details/`. Physical workers' MACs come from iLO Redfish, not from a deterministic formula.
