...
The **active OpenShift fleet membership** for the codex-opp-agent workspace is:
...
- A future operator-agent reads ADRs at session start. Without an ADR, the supersession lives only in memory and chat logs.
...
- **Workspace memory** (`/home/ze/.claude/projects/-home-ze-claude-agent/memory/`) updated to v6-only scope with an explicit decommissioned-DR guard. Memory entry: `project_workspace_scope.md`.
...
- **Memory entry `project_workspace_scope.md`** is now the operator-agent's fast-path source of truth for "what clusters are in scope right now." It duplicates the table above for ambient recall.

## References

- Source: `codex-opp-agent/adr/0022-v6-fleet-membership.md`
...
- Tracker: `zeshaq/codex-opp-agent` #156 (OPS-V6-FLEET-1 — the purge work)
- ADR Index issue: `zeshaq/codex-opp-agent` #131
- Memory entry: `project_workspace_scope.md`
