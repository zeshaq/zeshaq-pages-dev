...
description: "How architecture decisions are captured in this workspace — the template, the review-issue pair convention, and how they connect to GitHub issues, milestones, and memory."
sidebar_label: "Overview"
last_reviewed: 2026-05-11
draft: false
---

This section publishes the **Architecture Decision Records** (ADRs) that govern the lab platform. Each page below mirrors a source ADR file in `codex-opp-agent/adr/` and follows the same template: Context → Decision → Alternatives considered → Consequences → References. Readers who want the operating manual for *how* the platform is run should start at sections 2-5; this section explains *why* the operating manual looks the way it does.
...
In this workspace, ADRs are written in plain Markdown under `codex-opp-agent/adr/NNNN-short-slug.md`. Numbering is monotonic. The accepted set as of 2026-05-11 runs from `0001` to `0026`, with gaps allowed when a draft is abandoned before acceptance (none currently). Each ADR opens with a date and a `## Status` block — `Accepted`, `Superseded`, or `Proposed`. Bodies are dense paragraph prose, not bullet-only summaries, because the *why* is what readers need years later.
...
3. **ADR + review-issue pair.** When an ADR is drafted, a matching review issue is opened against the `codex-opp-agent` repo. The review issue is where reviewers (the operator role, the platform lead, the auditor — sometimes the same person across sessions) leave comments before the ADR moves to `Accepted`. The ADR file in `adr/` then references the review issue number.

The ADR Index issue (`codex-opp-agent` #131) maintains a flat list of every accepted ADR. When a new ADR supersedes an old one, the index issue records the supersession explicitly so the relationship is discoverable without grep.
...
sessions, incidents, or memory entries. The goal is that a reader who has
...
number, relevant GitHub issues and milestones, memory entries.>
```
...
For every ADR `NNNN-slug.md` there is an issue in `codex-opp-agent` titled along the lines of `adr: review NNNN <slug>` (or with the `WG`, `FG-`, `PCI-` phase prefix that triggered the work). The review issue holds:
...
## How ADRs interact with memory

The operator agent (Claude Code) maintains a small set of long-lived memory entries under `~/.claude/projects/-home-ze-claude-agent/memory/`. Memory entries are tactical reminders ("don't use `oc-mirror` for app images", "v6 fleet is hub-dc-v6 + spoke-dc-v6 only", "HAProxy is for VM-edge only"). Many of those reminders are one-line summaries of an ADR's decision. When the two disagree, the ADR wins — memory entries are convenience caches, not source of truth.

## How ADRs interact with the connection-details handbook

The `codex-opp-agent/connection-details/` directory is the operator handbook — `platform-admin-handoff.md`, `gitlab-operator-guide.md`, `nexus.md`, `vault-app-secrets.md`, and so on. These are the operating manuals; the ADRs are the constitution. Operating manuals follow the constitution. When a handbook says "do X this way" and an ADR says "X must work this way," the ADR is the binding statement and the handbook is the explanation of how to execute it.

## Reading order for this section

Page 2 is the **ADR Index** — every public ADR with a one-line summary, current status, and a small dependency graph so you can see which decisions supersede or refine which. Pages 3 through 17 are the individual ADRs in chronological order, sanitized for public publication. Where an ADR's content is highly operational (specific IPs, MAC addresses, passwords, internal-only hostnames), this site redacts to descriptive labels and points at the source file in `codex-opp-agent/connection-details/` for the internal-only specifics.
