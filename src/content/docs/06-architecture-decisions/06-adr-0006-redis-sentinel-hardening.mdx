...
- A three-node Sentinel-quorum cluster (`redis-0`, `redis-1`, `redis-2`) on dedicated private IPs in `30.30.30.0/24` (internal-only specifics in `codex-opp-agent/connection-details/`).
...
- **Production-readiness evidence lives in `codex-opp-agent/connection-details/redis.md`** (and its dated session reports). The closure of each gate is recorded with an `oc`/`redis-cli` output snippet showing the verified state.

## References

- Source: `codex-opp-agent/adr/0006-redis-sentinel-hardening.md`
...
- Operating notes: `codex-opp-agent/connection-details/redis.md` (when published; internal-only specifics meanwhile)
