...
The workspace now coordinates many parallel work streams: the OpenShift v6 rebuild, the developer-readiness contract, standalone VM-hosted platform services (Vault, Redis, Kafka, WSO2, Jenkins, SigNoz, monitoring, DefectDojo, Trivy), GitHub planning, ADRs, session memory, and the operator handoff documentation. The operator explicitly requested a **project-wide rule** so that work does not proceed as untracked chat-driven implementation.
...
- Milestone: `codex-opp-agent` #29 **Workspace Governance**.
- Phase issue: `codex-opp-agent` #106 **WG1: require issue, milestone, phase, and ADR before implementation**.
...
- planning changes (new ADR, new runbook, new memory entry),
...
3. **Update local session memory:** `CURRENT_STATE.md`, `TODO.md`, `SESSION_LOG.md`, and a detailed `reports/sessions/YYYY-MM-DD-<topic>.md` record.
...
This rule applies to Codex / operator work in this repository and to live platform work driven from this repository. It applies to:
...
- **Successful implementation is not complete** until the GitHub issue and local session memory are updated. The session is not considered closed until the closeout is written.
- **This rule is enforced primarily by the operator-agent (Claude Code).** The operator-agent reads `AGENTS.md` and the ADR Index at session start and reminds the operator when the rule is being skipped.

## References

- Source: `codex-opp-agent/adr/0016-workspace-execution-governance.md`
- Workspace milestone: `zeshaq/codex-opp-agent` #29 (Workspace Governance)
- Phase issue: `zeshaq/codex-opp-agent` #106 (WG1)
- Subagent-first workflow rule: `codex-opp-agent/adr/0002-subagent-first-workflow.md`
- Parallel-agent isolation rule: `codex-opp-agent/adr/0017-parallel-agent-worktree-isolation.md`
