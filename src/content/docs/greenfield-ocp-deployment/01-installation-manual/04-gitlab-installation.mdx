...
Do not register CI runners directly on the GitLab VM. The first runner should be a separate private-only VM:

| Item | Value |
| --- | --- |
| VM name | `gf-ocp-gitlab-runner-01` |
| Network | private-only `br33` |
| Suggested IP | `30.30.200.22` |
| Registration source | `platform/ci/gitlab-runner-config` |
| Token custody | Vault, under a future `secret/greenfield/gitlab/runners/<runner-name>` path |

Initial tags should be `gf`, `infra`, `ansible`, `tofu`, `packer`, and `openshift`.

Start with a shell executor on the private runner VM for infrastructure automation. Add Docker or Kubernetes executors only after registry, certificate trust, and runner isolation rules are defined.
