...
The GitLab VM is private-only. It has no real public interface. Package installation and future updates must use controlled private egress, such as NAT, an HTTP proxy, or an internal package mirror.
...
The cloud-init script installs the operating-system baseline, hardens SSH, enables UFW, adds the GitLab package repository, and installs GitLab CE. The private-only VM needs package access through NAT, a proxy, an internal mirror, or pre-staged packages before this script can complete.
...
- Private package egress still needs to be standardized through NAT, proxy, or an internal package mirror.
