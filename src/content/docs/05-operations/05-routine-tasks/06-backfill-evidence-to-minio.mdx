...
- **After a break-glass action** — the audit record under `codex-opp-agent/reports/break-glass/` is copied into MinIO for long-term retention.
...
1. **Confirm MinIO reachability.**
   ...
   ```bash
   . /home/ze/codex-opp-agent/secrets/minio-developer-ci-evidence.env
...
tar czf /tmp/evidence-stage/pci-baseline-2026-05-11.tar.gz \
  -C /home/ze/codex-opp-agent/reports/pci-dss/ \
...
```bash
cd /home/ze/codex-opp-agent/reports/pci-dss
...
- `codex-opp-agent/connection-details/minio.md` (the canonical MinIO endpoint + credential custody doc)
- ADR `0019-nexus-only-image-supply-chain.md` (image supply; MinIO is for evidence, not images)
- `codex-opp-agent/reports/pci-dss/` (the source compliance evidence packs)
- `codex-opp-agent/runbooks/break-glass-procedure.md` (break-glass audit records that also land here)
