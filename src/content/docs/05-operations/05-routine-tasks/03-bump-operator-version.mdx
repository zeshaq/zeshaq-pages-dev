...
Update `plans/disconnected-rebuild/environments/dc-lab/operator-version-lock.md` with the new pin (in a separate hunk inside the same MR for the version-lock file lives in `codex-opp-agent`, not `platform-gitops` — this is a cross-repo update; sequence the MRs).
...
9. `operator-version-lock.md` on `codex-opp-agent` reflects the new pin.
...
- `codex-opp-agent/connection-details/platform-admin-handoff.md` §"Operator Install Workflow" + §"Catalog Refresh Workflow"
- `codex-opp-agent/plans/disconnected-rebuild/environments/dc-lab/operator-version-lock.md`
- `codex-opp-agent/plans/disconnected-rebuild/environments/dc-lab/imageset-config.yaml`
- `codex-opp-agent/adr/0018-acm-openshift-gitops-pull-model-v6.md`, `0019-nexus-only-image-supply-chain.md`
