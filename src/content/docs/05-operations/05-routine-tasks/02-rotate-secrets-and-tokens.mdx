...
1. **Identify the credential's full consumer list.** A single password is often consumed by multiple subsystems (a Nexus bot is used by Jenkins; the RHACS Central admin is used by automation to generate init-bundles; the WSO2 super-admin is consumed by both APIM and IS). Make the list explicit.
   ...
   ```bash
   # Example: find all references to nexus-jenkinsbot-password in the workspace
   grep -rln "nexus-jenkinsbot-password" /home/ze/codex-opp-agent/ \
     | grep -v secrets/
   ```
...
mv /home/ze/codex-opp-agent/secrets/<file> \
   /home/ze/codex-opp-agent/secrets/<file>.pre-rotate-$TS
echo -n "$NEW_VALUE" > /home/ze/codex-opp-agent/secrets/<file>
chmod 600 /home/ze/codex-opp-agent/secrets/<file>
```
...
```bash
KCFG=/home/ze/codex-opp-agent/secrets/hub-dc-v6-kubeconfig
...
TS=$(date -u +%Y%m%dT%H%M%SZ)
mv /home/ze/codex-opp-agent/secrets/gitlab-zahid-pat \
   /home/ze/codex-opp-agent/secrets/gitlab-zahid-pat.pre-rotate-$TS
echo -n "<new-pat>" > /home/ze/codex-opp-agent/secrets/gitlab-zahid-pat
chmod 600 /home/ze/codex-opp-agent/secrets/gitlab-zahid-pat
...
# Update local mirror:
TS=$(date -u +%Y%m%dT%H%M%SZ)
mv /home/ze/codex-opp-agent/secrets/nexus-jenkinsbot-password \
   /home/ze/codex-opp-agent/secrets/nexus-jenkinsbot-password.pre-rotate-$TS
echo -n "$NEW_PASS" > /home/ze/codex-opp-agent/secrets/nexus-jenkinsbot-password
chmod 600 /home/ze/codex-opp-agent/secrets/nexus-jenkinsbot-password
