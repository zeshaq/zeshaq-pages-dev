...
3. **You have a GitLab identity with developer permission on `comptech-platform/openshift-ops/openshift-platform-gitops`** and a Personal Access Token stored at `codex-opp-agent/the operator GitLab PAT (local-only)`. The PAT is used by the API-driven MR flow; it does not need to be in your shell environment.
