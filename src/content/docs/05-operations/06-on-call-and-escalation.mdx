...
This page documents how on-call works on the lab. The lab is operated by a small team (currently solo with Claude-agent assistance per `user_role`), so "on-call" reads more as "the convention you follow when something is on fire" rather than a multi-tier PagerDuty rota. The conventions still matter — they keep recovery deterministic when the operator is tired, paged at 3 AM, or context-switching from another task.

The paging path, the escalation matrix, and the break-glass triggers are documented here. Per [section tracker #229](https://github.com/zeshaq/codex-opp-agent/issues/229), this page does not expose pager numbers, on-call phone numbers, or the rotation calendar — those live in the operator's local notes and the team-internal channels.

## The on-call posture

| Item | Value |
|---|---|
| Active rotation | Solo operator (workspace owner) + Claude-agent assistance |
...
1. An issue is opened on `zeshaq/codex-opp-agent` with the `incident` label (manually if the operator notices first; automatically as the alerting integrations mature).
...
Break-glass is the explicit policy in ADR 0025 governing live changes that bypass the [GitOps MR flow](/docs/05-operations/03-platform-gitops-mr-mechanics). The full procedure is in `codex-opp-agent/runbooks/break-glass-procedure.md`; this page is the operator-facing summary.
...
Every break-glass action produces a record under `codex-opp-agent/reports/break-glass/YYYY-MM-DD-<incident-slug>.md`. The template lives in `runbooks/break-glass-procedure.md`. The record captures:
...
- The GitLab PAT (`/home/ze/codex-opp-agent/secrets/gitlab-zahid-pat`) loaded into the curl wrapper.
- A scratch directory for `/tmp/break-glass-$(date -u +%Y%m%dT%H%M%SZ)` captures.
- The [day-1 handoff](/docs/05-operations/02-day-1-admin-handoff) and [break-glass procedure runbook](https://github.com/zeshaq/codex-opp-agent/blob/main/runbooks/break-glass-procedure.md) (the runbook lives in the workspace; this site has the operator-facing summary).
...
1. Write a dated session report under `codex-opp-agent/reports/sessions/`.
2. Close the incident issue with: timeline, root cause, fix, validation evidence, follow-up issues opened.
3. If the lesson is durable: update the relevant runbook in `codex-opp-agent/runbooks/`, the relevant published page in this section, the relevant memory entry under `~/.claude/projects/-home-ze-claude-agent/memory/`.
...
- `codex-opp-agent/adr/0025-gitops-only-operations-break-glass.md`
- `codex-opp-agent/runbooks/break-glass-procedure.md`
- `codex-opp-agent/connection-details/platform-admin-handoff.md` §"Break-Glass Rules"
- `codex-opp-agent/plans/federated-gitops-readiness-gates.md` (FG-6)
- Memory: `user_role`, `feedback_subagent_first`, `feedback_github_first_tracking`
