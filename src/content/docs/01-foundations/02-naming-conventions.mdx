...
See the memory entry `project_obc_to_operand_secret_bridge.md` for the canonical example.
...
The shared `push-image-quay` Tekton Task accepts the Secret name as a parameter, so each tenant overlay supplies its own robot token without per-task duplication. See the memory entry `reference_quay_robot_token_convention.md` for the canonical pattern.
...
- Memory: `reference_quay_robot_token_convention.md`, `project_obc_to_operand_secret_bridge.md`, `project_nexus_endpoint_split.md`
