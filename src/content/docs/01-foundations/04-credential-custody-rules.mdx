...
The operator workstation holds a local-only `secrets/` directory under `codex-opp-agent/secrets/` (the workspace boundary memory entry confirms this is the canonical location). The directory is:
...
The directory holds a handful of bootstrap-only credentials (Nexus admin and service-account passwords, the GitLab bootstrap PAT, per-cluster kubeadmin passwords, MinIO env files, the git-askpass shim). Exact filenames live in `codex-opp-agent/connection-details/` and are not enumerated here.
...
This pattern lives at `clusters/spoke-dc-v6/platform-services/tracing/externalsecret-tempo-storage.yaml` and is the reference for new operand-secret bridges (Loki backport tracked under issue #233). The memory entry `project_obc_to_operand_secret_bridge.md` documents the pattern.
...
- Memory: `feedback_workspace_boundary.md`, `project_obc_to_operand_secret_bridge.md`, `project_eso_egress_to_vault.md`, `reference_rhacs_init_bundle_via_api.md`, `reference_quay_robot_token_convention.md`
