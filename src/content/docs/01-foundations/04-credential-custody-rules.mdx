...
The operator workstation holds a local-only `secrets/` directory under `/home/ze/codex-opp-agent/secrets/` (the workspace boundary memory entry confirms this is the canonical location). The directory is:
...
Typical contents (names only, never values):

```text
nexus-admin-password
nexus-jenkinsbot-password
nexus-oauth2-cookie-secret
nexus-oidc-client.json
gitlab-md-pat
git-askpass.sh
hub-dc-v6-kubeadmin-password
spoke-dc-v6-kubeadmin-password
minio-developer-ci-evidence.env
minio-developer-ci-evidence-reader.env
```
