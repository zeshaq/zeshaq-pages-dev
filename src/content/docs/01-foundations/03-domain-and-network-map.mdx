...
<Mermaid client:load chart={`sequenceDiagram
  ...
  participant R as PowerDNS recursor
  participant A as PowerDNS authoritative
  V->>R: A? nexus-mirror.apps.sub.comptech-lab.com
  Note over R: forward-zones rule:<br/>sub.comptech-lab.com -> 127.0.0.1:53
  R->>A: A? nexus-mirror.apps.sub.comptech-lab.com
  A-->>R: A -> HAProxy edge address
  R-->>V: A -> HAProxy edge address
  V->>V: open TLS :443 to HAProxy edge
`} />
