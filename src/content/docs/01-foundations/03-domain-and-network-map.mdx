...
The OpenShift clusters issue their own per-cluster ingress wildcards on `*.apps.<cluster>.sub.comptech-lab.com`. Those are independent of HAProxy and are terminated by the cluster's ingress controller. **HAProxy never terminates OpenShift route traffic** (memory entry `feedback_haproxy_scope.md`).
...
- Memory: `reference_lab_infrastructure.md`, `reference_haproxy_vm.md`, `reference_pdns_vm.md`, `feedback_haproxy_scope.md`
