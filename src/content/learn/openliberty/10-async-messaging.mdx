...
> Note the image is the **official `apache/kafka`** image, not the
> historically-popular `bitnami/kafka:3.8` — Bitnami's docker.io
> distribution disappeared in the 2025 Broadcom reshuffle. The env
> vars also lose the `_CFG_` infix in the move from Bitnami's wrapper
> to the upstream entrypoint.
...
Without `kafka-clients`, Liberty fails app startup with `CWMRX1006E:
Classes from the kafka-clients JAR file could not be loaded`. The
...
Same topic, two channels — one for the producer side, one for the consumer side. The consumer side **also** needs the deserializer + offset-reset + commit-mode keys; without them, `@Incoming` deploys without error but silently fails to subscribe, no consumer group is ever registered, and no messages arrive. (See `build_gotchas` item 12 in the companion `insurance-app` repo memory.)
