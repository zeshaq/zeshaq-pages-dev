...
All of it runs as ~7 rootless `podman` containers on one VM, on one shared bridge network. By the end you'll have seen every layer of a real Java backend in motion — code, runtime, datastore, cache, event bus, identity, gateway, observability.
