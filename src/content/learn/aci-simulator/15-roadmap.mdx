...
| Lab | Name | Goal |
| --- | --- | --- |
| 8 | Cross-tenant shared services | Create `lab-shared` and deliberately share DNS/NTP-style services to approved tenants |
...
## Lab 8 starting point

Lab 8 should create a shared-services tenant:
...
| Application profile | `shared-services-app` |
| EPG | `dns-ntp-services` |
| Provided contracts | `shared-dns`, `shared-ntp` |
| Consumers | approved EPGs in `lab-prod` and `lab-dev` |

The important lesson is controlled sharing. Multi-tenant design starts isolated, then adds only the shared services that are explicitly required.
