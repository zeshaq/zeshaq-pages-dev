...
- **Federated identity provider.** Every cluster — hub and spokes — uses the same OAuth/OIDC source, so the same user logs into all of them with the same identity. Spoke-side `ClusterRoleBindings` reference the same `alice@corp` username (or group claim) that the hub uses. The lab takes this path with Keycloak: see [/docs/lab-infrastructure/other-platform-vms/keycloak-oidc](/docs/lab-infrastructure/other-platform-vms/keycloak-oidc). One IdP, every cluster trusts it, identities line up by name.
