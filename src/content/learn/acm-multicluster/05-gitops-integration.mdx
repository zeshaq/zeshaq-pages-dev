...
For the lab's `hub-dc-v6` + `spoke-dc-v6` pair this is the chosen design — see [ADR 0018](/docs/openshift-platform/architecture-decisions/adr-0018-acm-gitops-pull) and the [pull-model overview](/docs/openshift-platform/openshift-platform/gitops-operating-model/pull-model-overview) for the manifested decision.
...
Routes themselves survive — they live behind the aggregated APIService, not the CRD. `oc get routes -A` works throughout. Within seconds `/openapi/v2` recovers and Argo CD picks up on the next sync. The lab's [runbook for this incident](/docs/openshift-platform/operations/incidents-and-runbooks/acm-gitops-addon-routes-crd) tracks the permanent fix under issue #153.
...
The teaching moment: the convenience of a unified addon design has a real cost when one of the target environments has a native API the addon assumed wouldn't exist. Always check what the addon installs *as a CRD* against what your cluster already provides *as an APIService*. The lab's [gitops-operating-model docs](/docs/openshift-platform/openshift-platform/gitops-operating-model) cover the VAP guardrail in detail.
...
<Whiteboard client:load data={{
  nodes: [
    { id: "ctlpHdr", position: { x: 40,  y: 0   }, data: { label: "Control plane (hub)" }, style: { background: "#0c1812", border: "1px solid #15803d", borderRadius: 10, padding: 10, color: "#ffffff", width: 240, textAlign: "center", fontWeight: 700, fontSize: 12, letterSpacing: "0.03em", textTransform: "uppercase" } },
    { id: "git",     position: { x: 40,  y: 80  }, data: { label: "Git repo" }, style: ext },
    { id: "principal", position: { x: 40, y: 180 }, data: { label: "Argo CD principal\n(UI + Application CRs)" }, style: ctrl },
    { id: "agent",   position: { x: 40,  y: 300 }, data: { label: "Argo CD Agent process\n(on hub)" }, style: ctrl },
    { id: "wlHdr",   position: { x: 460, y: 0   }, data: { label: "Workload cluster (spoke)" }, style: { background: "#0c1812", border: "1px solid #15803d", borderRadius: 10, padding: 10, color: "#ffffff", width: 240, textAlign: "center", fontWeight: 700, fontSize: 12, letterSpacing: "0.03em", textTransform: "uppercase" } },
    { id: "spokeAgent", position: { x: 460, y: 100 }, data: { label: "Argo CD Agent\n(spoke side)" }, style: agent },
    { id: "spokeArgo",  position: { x: 460, y: 220 }, data: { label: "Lightweight Argo CD\n(local reconcile)" }, style: agent },
    { id: "spokeApi",   position: { x: 460, y: 340 }, data: { label: "Spoke kube-apiserver" }, style: stage },
  ],
  edges: [
    { id: "ag1", source: "principal", target: "agent", style: { stroke: "#1f2937" } },
    { id: "ag2", source: "principal", target: "git", label: "pulls", style: { stroke: "#15803d", strokeDasharray: "5,5" }, animated: true },
    { id: "ag3", source: "spokeAgent", target: "agent", label: "mTLS .spec down / .status up", style: { stroke: "#15803d", strokeDasharray: "5,5" }, animated: true },
    { id: "ag4", source: "spokeAgent", target: "spokeArgo", style: { stroke: "#1f2937" } },
    { id: "ag5", source: "spokeArgo", target: "spokeApi", label: "reconcile", style: { stroke: "#1f2937" } },
  ]
}} height={460} />
...
| | **Managed mode** | **Autonomous mode** |
| --- | --- | --- |
| `.spec` owner | Principal (hub) | Workload cluster (spoke) |
| `.status` direction | Spoke → principal | Spoke → principal |
| Who creates Applications | Hub operator via principal UI/CLI | Spoke team via spoke Git |
| Hub outage during sync | Blocks new sync | Spoke continues; status backlog clears on reconnect |
| Best for | Tightly-governed fleets, central platform team owns rollout | Independent ops teams per site, GitOps purist setup |
...
| Concern | gitops-addon (current lab) | Argo CD Agent (new direction) |
| --- | --- | --- |
| Source-of-truth for Application CR | Hub Argo CD (always) | Principal in managed; spoke in autonomous |
| Spoke footprint | Full OpenShift GitOps install | Agent + slimmer Argo CD |
| Courier for spec | ACM ManifestWork | Purpose-built mTLS channel |
| Disconnected spoke resilience | Eventual; depends on ManifestWork retry | Better; agent reconnects independently |
| OpenShift dependency | OpenShift only (uses Routes, OLM, ACM) | OpenShift GitOps 1.19+; not tied to ACM |
| GA status | GA, broadly deployed since 2024 | GA in OpenShift GitOps 1.19 (late 2025) |
| Multi-mode (managed/autonomous) | No | Yes |
| Scales comfortably past ~30 spokes | Increasing operational cost | Designed for it |
...
The earlier section in this module documented the Routes CRD that gitops-addon installs on every spoke and the OpenAPI v2 collision it causes on OpenShift. **Argo CD Agent does not (currently) hit the same trap** — its spoke side is a slimmer install that doesn't ship the same CRD bundle. That said, the discipline is the same: any addon you install onto an OpenShift spoke can collide with a native aggregated APIService, and "the cluster looks healthy on every other angle" is exactly how this class of bug presents. After any new addon install, run the lab's CRD-inventory check (see the [Routes CRD runbook](/docs/openshift-platform/operations/incidents-and-runbooks/acm-gitops-addon-routes-crd)) and confirm no duplicate API surfaces.
...
- [docs.redhat.com — Argo CD Agent architecture](https://docs.redhat.com/en/documentation/red_hat_openshift_gitops/1.19/html/argo_cd_agent_architecture/)
- [docs.redhat.com — Argo CD Agent installation and configuration](https://docs.redhat.com/en/documentation/red_hat_openshift_gitops/1.19/html/installing_argo_cd_agent/)
- [docs.redhat.com — Red Hat OpenShift GitOps 1.19 release notes](https://docs.redhat.com/en/documentation/red_hat_openshift_gitops/1.19/html/release_notes/)
- [argo-cd.readthedocs.io — Applications in any namespace](https://argo-cd.readthedocs.io/en/stable/operator-manual/app-any-namespace/)
...
The lab's [pull-model overview](/docs/openshift-platform/openshift-platform/gitops-operating-model/pull-model-overview) has the full diagram and the seven manifests under `fleet-registration/`. The ADR rationale is in [ADR 0018](/docs/openshift-platform/architecture-decisions/adr-0018-acm-gitops-pull).
...
- Lab docs — Pull-model overview: [/docs/openshift-platform/openshift-platform/gitops-operating-model/pull-model-overview](/docs/openshift-platform/openshift-platform/gitops-operating-model/pull-model-overview)
- Lab docs — Routes CRD incident runbook: [/docs/openshift-platform/operations/incidents-and-runbooks/acm-gitops-addon-routes-crd](/docs/openshift-platform/operations/incidents-and-runbooks/acm-gitops-addon-routes-crd)
- Lab docs — ADR 0018 ACM + GitOps pull: [/docs/openshift-platform/architecture-decisions/adr-0018-acm-gitops-pull](/docs/openshift-platform/architecture-decisions/adr-0018-acm-gitops-pull)
- Lab docs — Spoke RBAC extensions: [/docs/openshift-platform/openshift-platform/gitops-operating-model/spoke-rbac-extensions](/docs/openshift-platform/openshift-platform/gitops-operating-model/spoke-rbac-extensions)
