...
For the lab's enable/disable runbook, see [`/docs/openshift-platform/acm-multicluster/acm-cluster-backup/`](/docs/openshift-platform/acm-multicluster/acm-cluster-backup).
...
For the lab's install recipe and the `DataProtectionApplication` pattern, see [`/docs/openshift-platform/backup-oadp/operator-install/`](/docs/openshift-platform/backup-oadp/operator-install).
...
- **ACM cluster-backup is disabled on hub-dc-v6.** It will be enabled when the DR pair is built. The runbook in [`/docs/openshift-platform/acm-multicluster/acm-cluster-backup/`](/docs/openshift-platform/acm-multicluster/acm-cluster-backup) walks through the enable.
...
- **hub-dr-v6** is reserved in the fleet design but not yet provisioned. It is the planned restore target. See [`/docs/architecture-decisions/adr-0022-v6-fleet-purge/`](/docs/architecture-decisions/adr-0022-v6-fleet-purge) for the architectural decision.
...
The BFSI readiness review rates DR as 🔴 High — the missing DR pair is the load-bearing gap. See [`/docs/foundations/bfsi-readiness-review/`](/docs/foundations/bfsi-readiness-review).
...
The BFSI readiness review treats DR as a **🔴 High** risk so long as the reserved DR pair (hub-dr-v6) is not built. The criteria are spelled out at [`/docs/foundations/bfsi-readiness-review/`](/docs/foundations/bfsi-readiness-review). Until then, the lab's posture is single-DC with rebuild-from-GitOps as the fallback — adequate for a lab, not adequate for a regulated tenant.
