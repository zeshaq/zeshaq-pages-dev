...
The lab runs Red Hat's Gatekeeper Operator on `spoke-dc-v6` already — see [/docs/openshift-platform/platform-services/gatekeeper](/docs/openshift-platform/platform-services/gatekeeper). The install was hand-rolled because there was only one spoke; if a second spoke ever arrives, the install gets wrapped in an ACM Policy and fanned out via Placement.
