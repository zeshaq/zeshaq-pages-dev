...
export const branch = (header, angleDeg, leaves, cx, cy) => {
  ...
  // Direction vector along the branch (forward) and perpendicular to it
  ...
  nodes.push({
    id: headerId,
    position: { x: headerCenter.x - 90, y: headerCenter.y - 25 },
    data: { label: header },
    style: branchHeader,
  });
  edges.push({
  ...
  // Leaves in two columns extending outward from the header along the branch direction.
  // FORWARD_STEP must exceed the leaf's longest dimension (~130px wide) so that
  // east/west branches don't stack overlapping cards along the x-axis.
  const FORWARD_START = 130;
  const FORWARD_STEP = 165;
  const PERP_OFFSET = 90;
