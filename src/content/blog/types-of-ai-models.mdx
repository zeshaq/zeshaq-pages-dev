...
<Mermaid client:load chart={`flowchart LR
  Web[Web corpora] --> Mix[Data mix + filtering]
  Books[Books + papers] --> Mix
  Code[Code repos] --> Mix
  Synth[Synthetic data] --> Mix
  Mix --> PT[Pre-training: next-token prediction]
  PT --> Base[Base model]
  Base --> SFT[Supervised fine-tuning]
  SFT --> RLHF[RLHF / DPO / RLAIF]
  RLHF --> Safety[Safety + refusal training]
  Safety --> Released[Released model]

  classDef phase fill:#e5e7eb,stroke:#6b7280,color:#1f2937
  classDef alert fill:#fafafa,stroke:#15803d,color:#1f2937
  class Mix,PT,SFT,RLHF,Safety phase
  class Web,Books,Code,Synth,Base,Released alert`} />
...
<Mermaid client:load chart={`flowchart LR
  Q[Request] --> R{Router}
  R -- simple --> Cheap[Cheap generalist LLM]
  R -- complex --> Reasoning[Reasoning model]
  R -- needs world knowledge --> RAG[Generalist + RAG]
  Cheap --> Out[Response]
  Reasoning --> Out
  RAG --> Out
  ...
  classDef phase fill:#e5e7eb,stroke:#6b7280,color:#1f2937
  classDef alert fill:#fafafa,stroke:#15803d,color:#1f2937
  class R,Cheap,Reasoning,RAG phase
  class Q,Out alert`} />
...
<Mermaid client:load chart={`flowchart LR
  ...
  ASR --> Buf[Partial transcript buffer]
  Buf -- on endpoint --> LLM[LLM with tool use]
  LLM --> TTS[Streaming TTS]
  TTS --> Spk[Speaker]
  LLM -- tool calls --> Backend[Backend systems]
  Backend --> LLM

  classDef phase fill:#e5e7eb,stroke:#6b7280,color:#1f2937
  classDef alert fill:#fafafa,stroke:#15803d,color:#1f2937
  class VAD,ASR,Buf,LLM,TTS,Backend phase
  class Mic,Spk alert`} />
...
<Mermaid client:load chart={`flowchart TD
  User[User input] --> Vision{Image?}
  Vision -- yes --> VLM[Vision-Language Model]
  Vision -- no --> Embed[Embedding Model]
  VLM --> Embed
  Embed --> VS[(Vector Store)]
  VS --> Retrieve[Retrieve top-K]
  Retrieve --> Router{Router}
  Router -- simple --> LLM[Generalist LLM]
  Router -- hard --> Reason[Reasoning Model]
  LLM --> Tool{Tool call?}
  Reason --> Tool
  Tool -- yes --> Tools[Execute tool]
  Tools --> LLM
  Tool -- no --> Resp[Response]

  classDef phase fill:#e5e7eb,stroke:#6b7280,color:#1f2937
  classDef alert fill:#fafafa,stroke:#15803d,color:#1f2937
  class VLM,Embed,VS,Retrieve,Router,LLM,Reason,Tool,Tools phase
  class User,Resp alert`} />
